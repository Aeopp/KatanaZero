...
HRESULT Multi_Texture::Insert_Texture(const wstring & FilePath, const wstring & StateKey, const uint32_t Count)
{
...
std::shared_ptr<class TexInfo> Multi_Texture::Get_TexInfo(const wstring & StateKey, const uint32_t Index)
{
	auto& iter_find = _TextureMap.find(StateKey);
